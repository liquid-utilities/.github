# Contributing to Liquid Utilities
[heading__title]:
  #contributing-to-liquid-utilities
...
The text within this document are not _set in stone_ rules but guidelines, and much like other files maintained by Liquid Utilities the following may be edited via a _`Pull Request`_.
...
Review the [whole thing][branch__current__code_of_conduct] to see what is expected of those that maintain code with Liquid Utilities
...
Join the <sub>[![Contributors][badge__contributors]][branch__current__network_members]</sub> issuing Pull Requests that close available <sub>[![Open Issues][badge__issues]][branch__current__issues]</sub>. [New Issues][branch__current__issues_new] may be opened for Reporting Bugs and Suggesting Enhancements. However, please search for existing similar Issues first; example search for [`memory-leaks`](https://github.com/liquid-utilities/.github/search?q=memory-leaks&type=Issues) from the `.github` repository.
...
```Bash
_git_name='your-name'
_organization='liquid-utilities'
...
set _organization='liquid-utilities'
...
There may be _`orphan`_ branches utilized by liquid-utilities for separating Code to be included in other projects from Documentation and Usage Examples. Non-orphaned development branches are encouraged when adding features or fixing bugs.
...
```
Dedicated to building Liquid tools for static web sites, such as those hosted by GitHub Pages.
...
[branch__current__network_members]:
  network/members
  "Perhaps you too may be counted amount those that have Forked repositories maintain by Liquid Utilities"

[branch__current__support]:
  SUPPORT.md
  "Best avenues for seeking support of Liquid Utilities"
...
[badge__issues]: https://img.shields.io/github/issues/liquid-utilities/.github.svg

[badge__contributors]: https://img.shields.io/github/forks/liquid-utilities/.github.svg?color=005571&label=Contributors
